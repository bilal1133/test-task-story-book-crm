...
export const getPathForAllUserData = (): string => Collection.UserData;

export const getPathForSingleUserData = (userId: DBUser['uid']): string => `${getPathForAllUserData()}/${userId}`;
...
export const getPathForUserBusiness = (userId: DBUser['uid']): string => `${getPathForAllUserData()}/${userId}/crm-businesses/${'abc'}/`;
// ^^^^ BEWARE: use 'getPathForSingleUserData' instead of 'getPathForMyUserData' because 'getUserId' returns 'undefined' when the hook returns 'user'
